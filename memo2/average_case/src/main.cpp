...
int main() {
	constexpr std::size_t WING_TOTAL = 20;
	...
	auto bar = barkeep::ProgressBar(&loops, {
		.total = total,
	...
	data.reserve(TRIAL_TOTAL * WING_TOTAL * WING_TOTAL);
				...
				for (std::size_t supply_count = 1; supply_count <= wing_count; ++supply_count) {
					for (std::size_t supply_storage_size = 1; supply_storage_size <= supply_count; ++supply_storage_size) {
						for (std::size_t trials = 0; trials < TRIAL_TOTAL; ++trials) {
							Facility facility{static_cast<int>(trials + TRIAL_TOTAL * (wing_count + WING_TOTAL * (exit_count + wing_count * (supply_count + wing_count * supply_storage_size)))), wing_count, exit_count, supply_count};
							std::unordered_map<Graph::VertexT, SupplyID> vertex_to_supply_id;
							for (int i = 1; const auto s : facility.supplies) {
								vertex_to_supply_id[s] = ++i;
							}
							std::vector<SupplyID> supply_storage(supply_storage_size, 0);
							auto path = ember_rescue(std::make_pair(facility.wings, facility.junctions), facility.entry, facility.supplies, facility.exits, supply_storage, vertex_to_supply_id, empty_set);
							std::size_t v = 0, e = 0;
...
#pragma omp single
				{
					std::ofstream file{"data.txt", std::ios::binary | std::ios::app};
